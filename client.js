...
function renderSingleVidReq(vidInfo, isPrepend = false) {
  ...
  vidReqContainerElm.innerHTML = `<div class="card mb-3">
    ${
      state.isSuperUser
        ? `<div class="crad-header d-flex justify-content-between">
  ...
  const adminSvaeVideoResElm = document.getElementById(
  ...
  if (state.isSuperUser) {
    ...
    adminSvaeVideoResElm.addEventListener("click", (e) => {
      e.preventDefault();
      ...
      updateVideoStatus(vidInfo._id, e.target.value, adminVideoResElm.value);
    });
    ...
    adminDeleteVideoReqElm.addEventListener("click", (e) => {
      ...
      const isSure = confirm(
        `Are you sure you want to delete "${vidInfo.topic.topic_title}"`
      );
